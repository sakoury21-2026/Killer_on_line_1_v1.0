using UnityEngine;
// נותן גישה ל-MonoBehaviour, ל-Vector2, ל-Vector3, ל-Mathf ול-Time של Unity.
using UnityEngine.InputSystem;
// נותן גישה לעכבר דרך מערכת הקלט החדשה של Unity.

[DisallowMultipleComponent]
// מונע חיבור של שתי מערכות מגירה לאותו חלק נע.
public class DrawerInteractable : MonoBehaviour, IHoldInteractable
// מגדיר מגירה שמתחילה לפעול בלחיצה על E ומפסיקה בשחרור E.
{
    // פתיחת גוף המחלקה DrawerInteractable.
    [Header("Drawer Movement")]
    // יוצר כותרת ב-Inspector עבור ערכי התנועה של המגירה.
    [SerializeField] private float openDistance = 0.35f;
    // קובע כמה מטרים המגירה יכולה לנוע מהמיקום הסגור למיקום הפתוח.
    [SerializeField] private bool opensInPositiveZ = true;
    // קובע האם כיוון הפתיחה הוא Z חיובי או Z שלילי ביחס ל-Parent של המגירה.
    [SerializeField] private float dragSensitivity = 0.001f;
    // ממיר את תנועת העכבר למרחק תנועה של המגירה.

    [Header("Player References")]
    // יוצר כותרת ב-Inspector עבור החיבורים לרכיבי השחקן.
    [SerializeField] private PlayerLook playerLook;
    // שומר חיבור לרכיב שמסובב את המצלמה כדי שנוכל לנעול אותו בזמן האחיזה.
    [SerializeField] private PlayerMovement playerMovement;
    // שומר חיבור לרכיב שמזיז את השחקן כדי שנוכל לנעול אותו בזמן האחיזה.

    [Header("Noise")]
    // יוצר כותרת ב-Inspector עבור הגדרות הרעש של המגירה.
    [SerializeField] private NoiseSystem noiseSystem;
    // שומר חיבור למערכת שמפרסמת את אירועי הרעש.
    [SerializeField] private float drawerNoiseRadius = 5f;
    // קובע שהרעש של המגירה נשמע ברדיוס התחלתי של חמישה מטרים.
    ...
    // קובע כמה שניות חייבות לעבור לפני שמותר לדווח שוב על רעש מאותה מגירה.

    private float closedZ;
    // שומר את מיקום Z המקומי של המגירה כשהיא סגורה.
    private float openZ;
    // שומר את מיקום Z המקומי של המגירה כשהיא פתוחה עד הסוף.
    private float openingDirection;
    // שומר 1 לפתיחה ב-Z חיובי או מינוס 1 לפתיחה ב-Z שלילי.
    private float nextAllowedNoiseTime;
    // שומר את הזמן המוקדם הבא שבו מותר לפרסם אירוע רעש.
    private bool isHeld;
    // שומר האם השחקן מחזיק כרגע את המגירה באמצעות E.

    private void Awake()
    // פועל פעם אחת כאשר ה-GameObject נטען ומכין את כל הנתונים הדרושים למגירה.
    {
        // פתיחת המתודה Awake.
        if (playerLook == null)
        ...
        // בודק אם גם החיפוש האוטומטי לא מצא PlayerLook.
        {
            ...
            Debug.LogError("לא נמצא PlayerLook בסצנה", this);
            // מציג שגיאה ומקשר אותה למגירה הבעייתית.
        }
        ...
        // בודק אם גם החיפוש האוטומטי לא מצא PlayerMovement.
        {
            ...
            Debug.LogError("לא נמצא PlayerMovement בסצנה", this);
            // מציג שגיאה ומקשר אותה למגירה הבעייתית.
        }
        ...
        // בודק אם גם החיפוש האוטומטי לא מצא NoiseSystem.
        {
            ...
            Debug.LogError("לא נמצא NoiseSystem בסצנה", this);
            // מציג שגיאה כי המגירה לא תוכל לדווח רעש.
        }
        // סיום תנאי השגיאה של NoiseSystem.

        closedZ = transform.localPosition.z;
        // שומר את מיקום הסגירה מתוך המיקום שבו המגירה הונחה ב-Prefab או בסצנה.
        openingDirection = opensInPositiveZ ? 1f : -1f;
        // מתרגם את בחירת הכיוון ב-Inspector למספר שבו נוכל להשתמש בחישוב.
        openZ = closedZ + openDistance * openingDirection;
        // מחשב את גבול הפתיחה המרבי של המגירה.
    }
    // סיום המתודה Awake.

    private void Update()
    // פועל בכל פריים ומזיז את המגירה רק בזמן שהשחקן מחזיק אותה.
    {
        // פתיחת המתודה Update.
        if (!isHeld)
        // בודק אם אין כרגע אחיזה פעילה במגירה.
        {
            // פתיחת תנאי האחיזה.
            return;
            // עוצר את Update של המגירה כי אסור להזיז אותה ללא החזקת E.
        }
        // סיום תנאי האחיזה.
        ...
        // קורא כמה העכבר זז מאז הפריים הקודם בציר X ובציר Y.
        TryReportRoughMovement(mouseDelta);
        // בודק אם המשיכה הייתה מהירה או עקומה ומדווח רעש במקרה הצורך.

        float drawerMovement = -mouseDelta.y * dragSensitivity * openingDirection;
        // הופך תנועת עכבר אנכית לשינוי במיקום המגירה בכיוון הפתיחה הנבחר.
        Vector3 drawerPosition = transform.localPosition;
        // יוצר עותק של המיקום המקומי הנוכחי כדי לשנות רק את ציר Z.
        float minimumZ = Mathf.Min(closedZ, openZ);
        // מוצא את גבול Z הנמוך בין המצב הסגור למצב הפתוח.
        float maximumZ = Mathf.Max(closedZ, openZ);
        // מוצא את גבול Z הגבוה בין המצב הסגור למצב הפתוח.
        drawerPosition.z = Mathf.Clamp(drawerPosition.z + drawerMovement, minimumZ, maximumZ);
        // מוסיף את תנועת העכבר ומונע מהמגירה לעבור את גבולותיה.
        transform.localPosition = drawerPosition;
        // מחיל בפועל את המיקום החדש על המגירה.
    }
    // סיום המתודה Update.
    ...
    // מקבלת את תנועת העכבר ובודקת אם היא מהירה מדי או נוטה הצידה.
    {
        ...
        // מונע חלוקה באפס ומשתמש בזמן שאינו מושפע מהאטה או עצירה של המשחק.
        float pullSpeed = Mathf.Abs(mouseDelta.y) / safeDeltaTime;
        // ממיר את תנועת העכבר האנכית בפיקסלים למהירות חיובית בפיקסלים לשנייה.
        bool pulledTooFast = pullSpeed >= fastPullThreshold;
        // שומר true אם מהירות המשיכה עברה את הסף שקבענו.
        bool pulledSideways = Mathf.Abs(mouseDelta.x) >= sidewaysNoiseThreshold;
        // שומר true אם היד זזה הצידה יותר מהסף ולכן המשיכה נחשבת עקומה.
        ...
        // בודק אם המשיכה הייתה גם איטית וגם ישרה.
        {
            ...
            return;
            // יוצא בלי רעש כי השחקן השתמש במגירה בעדינות.
        }
        ...
        // קובע מתי יהיה מותר לפרסם את אירוע הרעש הבא.
        noiseSystem?.ReportNoise(transform.position, drawerNoiseRadius);
        // מדווח את מיקום המגירה ורדיוס הרעש אם מערכת הרעש קיימת.
    }
    ...
    // נקראת על ידי PlayerInteraction כאשר פעולת Interact מגיעה לשלב performed.
    {
        ...
        isHeld = true;
        // מסמן שהמגירה מוחזקת ולכן Update רשאי לקרוא את תנועת העכבר.
        playerLook.SetLookEnabled(false);
        // נועל את סיבוב המצלמה כדי שהעכבר ישלוט במגירה.
        playerMovement.SetMovementControlsEnabled(false);
        // נועל הליכה, ריצה וכריעה בזמן השימוש במגירה.
    }
    ...
    // נקראת על ידי PlayerInteraction כאשר פעולת Interact מגיעה לשלב canceled.
    {
        ...
        // בודק אם אין אחיזה פעילה שצריך לסיים.
        {
            ...
            return;
            // יוצא בלי פעולה כדי לא לשנות שליטה שלא ננעלה על ידי המגירה הזאת.
        }
        // סיום תנאי האחיזה הלא פעילה.

        isHeld = false;
        // מסמן שהמגירה כבר אינה מוחזקת ולכן Update מפסיק להזיז אותה.
        ...
        // בודק שרכיב התנועה עדיין קיים.
        {
            ...
            // מחזיר לשחקן את פקודות ההליכה, הריצה והכריעה.
        }
    ...
    // פועלת אם המגירה או הסקריפט נכבים בזמן שהשחקן עדיין מחזיק E.
    {
